...
# default method
CSC45.formula <- 
  function(form, data, pruning = TRUE, MDL = TRUE, minIns = 2, costRatio  = 11/56)
  {           
    # Input:
    #     form      :  formula 
    #     data      :  training data
    #     pruning   :  prune the decision tree
    #     minIns    :  minimum number of instances for split
    #     costRatio :  costRatio between Majority class and Minority class
    
    # input check 
    Call <- match.call()
    indx <- match(c("form", "data"), names(Call), nomatch = 0L)
    if (indx[1] == 0L | indx[2] == 0L) stop("'formula' and 'data' arguments are required")
    ...
    dataY       <- data[, tgt]
    numRow      <- dim(dataX)[1]
    numColumn   <- dim(dataX)[2]
    classTable  <- table(dataY)
    classTable  <- sort(classTable, decreasing = TRUE)
    classLabels <- names(classTable)
    indexMajorIns <- dataY == classLabels[1] 
    ...
    isOrdered     <- sapply(dataX, is.ordered)
    isFactor      <- sapply(dataX, is.factor)
    ...
    { 
      label <- dataY[insIndex]
      att   <- dataX[insIndex, attIndex] 
    ...
    {
      classDistribution <- table(dataY[insIndex])
      ...
      {  
        ...
        if (maxCategoricalEntropy > maxContinousEntropy){
          ...
          splitRule   <- dataX[insIndex, currentCategorical[splitAttIndex]]
        ...
        if (maxContinousEntropy > maxCategoricalEntropy){
          ...
          splitRule <- dataX[insIndex, currentContinous[splitAttIndex]] > splitInfo$splitPoint
          attNext   <- attIndex
...
predict.CSC45 <- function(object, data, type = "class")
{
  Call <- match.call()
  indx <- match(c("object", "data"), names(Call), nomatch = 0L)
  if (indx[1] == 0L | indx[2] == 0L) stop("'object' and 'data' arguments are required")
  ...
  insIndex   <- c(1 : numRow)
  rootIndex  <- length(tree)
  yName      <- as.character(object$formula[[2]])
  tgt <- match(yName, names(data), nomatch = 0L)
  nodeCodeList <- sapply(tree, '[[', 1)
  if (tgt != 0)
    data <- data[, -tgt] 
