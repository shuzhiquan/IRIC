...
bbaging.formula <-
  function(form, data, numBag = 40, base = treeBag, type = "Bagging", allowParallel = TRUE, ...)
  {
    # Input:
    #    form: model formula
    ...
    #    base: base learner
    #    type: SMOTEBagging, RUSBagging, RBBagging
    ...
    if (!type %in% c("Bagging", "RUSBagging", "ROSBagging", "SMOTEBagging", "RBBagging"))
...
predict.bbag<-    
  ...
  {
    if(is.null(data)) stop("please provide a data set for prediction")
